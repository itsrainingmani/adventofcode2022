...
//                         items     op       val     by  true   false
#[derive(Debug)]
struct NoWorryMonkey(VecDeque<u64>, String, String, u64, usize, usize);

fn process_part1() -> Result<(), Box<dyn Error>> {
    let monkey_re: Regex = Regex::new(r"Monkey (?P<index>[0-9]+)").unwrap();
    let items_re: Regex = Regex::new(r"Starting items: (?P<items>.+)").unwrap();
    let op_re: Regex = Regex::new(r"Operation: new = old (?P<op>.) (?P<val>.*)").unwrap();
    let test_re: Regex = Regex::new(r"Test: divisible by (?P<div_by>[0-9]+)").unwrap();
    let true_re: Regex = Regex::new(r"If true: throw to monkey (?P<index>[0-9]+)").unwrap();
    let false_re: Regex = Regex::new(r"If false: throw to monkey (?P<index>[0-9]+)").unwrap();
...
fn process_part2() -> Result<(), Box<dyn Error>> {
    ...
    let input = read_to_string("inputs/day11.txt")?;
    let num_monkeys = input.matches("Monkey").collect::<Vec<&str>>().len();
    let mut monkeys: Vec<NoWorryMonkey> = vec![];
    let mut item_stack: HashMap<usize, VecDeque<u64>> = HashMap::new();

    let mut monkey_count: Vec<u64> = vec![];
    for i in 0..num_monkeys {
        monkeys.push(NoWorryMonkey(
            VecDeque::new(),
            String::new(),
            String::new(),
            0,
    ...
    for _ in 0..10_000 {
        for (cur_idx, monk) in monkeys.iter_mut().enumerate() {
            ...
            while !monk.0.is_empty() {
                ...
                if monk.2 == "old" {
                    if monk.1 == "*" {
                        item_worry = (item * item).rem(supermodulo);
                    } else {
                        item_worry = (item + item).rem(supermodulo);
                    }
                } else {
                    let val = monk.2.parse::<u64>().unwrap();
                    if monk.1 == "*" {
                        item_worry = (item * val).rem(supermodulo);
                    } else {
                        item_worry = (item + val).rem(supermodulo);
                    }
                }
